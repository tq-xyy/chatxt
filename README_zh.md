...
路径相对于 `.chat.txt` 所在目录解析；被 include 文件内部的指令相对于该文件所在目录解析。

| 指令             | 说明                                                                                                                                           |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `@file(路径)`    | 将外部文件内容追加到该 user 消息尾部（同一文件一次会话只引用一次，重复引用仅保留文件名）                                                       |
| `@include(路径)` | 将外部文件作为预设递归展开：文件内容会被重新解析为指令（`@file` / `@tool` / `@include`，支持嵌套，循环引用跳过并警告）；含角色行的文件会被拒绝 |
...
被引用的预设（`presets/web-search.txt`）内部可以继续声明工具与文件：
